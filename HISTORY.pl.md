# GhostMEAN — Pełna historia projektu
...
<p align="center"><em>🇬🇧 <a href="HISTORY.md">English version</a> · 🏠 <a href="README.pl.md">Powrót do README</a></em></p>

Każda wersja, w pełnym szczególe, od pierwszego prototypu do aktualnego
wydania. Skrócony przegląd znajdziesz w [README.pl.md](README.pl.md);
jak faktycznie korzystać z każdej funkcji — w
[Wiki](https://github.com/nftomczain/GhostMEAN/wiki).

## Historia zmian

### v1.0.1 — Wiki, realne fixtury CSV, podział README

- **`docs/wiki/`** — 9 gotowych do wklejenia stron GitHub Wiki (Home, Installation, Quick-Start, Panel-Geometry, Station-View, CSV-Format, Validation, Export-PDF, Troubleshooting), po angielsku. Wszystkie fakty zweryfikowane przeciwko rzeczywistemu kodowi, nie zgadywane z pamięci: dokładnie sprawdziłem `CSV_FIELDS`/`STATIONS_CSV_FIELDS`, próg dużego skosu (60°), domyślne fallbacki metadanych CSV, listę 6 języków — oraz uruchomiłem `tests/fixtures/test_err.csv` na żywym GUI, żeby potwierdzić, że wszystkie sześć opisanych w Wiki typów ostrzeżeń faktycznie się pojawia (w tym dwa osobne skoki geometrii w tym samym pliku). Strona Troubleshooting uczciwie oznacza niepotwierdzone rzeczy jako niepotwierdzone: obejście MIT-SHM (`QT_XCB_NO_MITSHM=1`) opisane jako "pasuje do znanej klasy błędu X11", nie jako gwarantowana naprawa.
- **Realne fixtury CSV od użytkownika**: `tests/fixtures/test.csv` teraz ze skosem (20°/10°/5°), dokładnie zgodny z przykładem z Quick Start (sprawdzone: skos nie zmienia MAC — 189.621mm bez zmian). `tests/fixtures/test2.csv` całkowicie przerobiony na ciągłe 5 paneli, `320→270→210→160→110→70mm`, ze zmiennym skosem — przeliczone i zablokowane jako regresja (MAC=215.681mm, Span=2200mm, Area=422500mm², 6 stacji, zero skoków). `tests/fixtures/test_err.csv` zaktualizowany (mniej paneli, jeden skok geometrii zamiast dwóch) — wszystkie dotychczasowe asercje przeszły bez zmian. Opis `test2.csv` w `docs/wiki/CSV-Format.md` poprawiony na zgodny z nową zawartością.
- **`README.md` / `README.pl.md` napisane od nowa, odchudzone**: co to jest, screenshot, funkcje, instalacja, szybki start, dostępne języki, testy, pobieranie/release, Wiki, licencja — nic więcej. Z 420 linii do 120 (EN) / z 368 do 122 (PL). **Powstały `HISTORY.md` / `HISTORY.pl.md`** — pełna, wersja-po-wersji historia zmian, przeniesiona dosłownie ze starego README, zero utraconej treści, plus dwa aneksy (Matematyka, Co dalej). **Dodany `docs/screenshot.png`** — prawdziwy zrzut ekranu (przykład Quick Start, 3 panele, wymiary włączone) — README nigdy wcześniej go nie miał.
- 97/97 testów zielone przez całą turę.

### v0.4.11 — testy + `--version`

Zgodnie z planem "feature freeze → testy → release": zero nowych funkcji, tylko utwardzanie tego, co już jest.

- **`ghostmean --version`** — nowy alias `ghostmean` (obok `ghostmean-gui`), obie komendy obsługują `--version`/`-v`, wypisują `GhostMEAN <wersja>` i kończą działanie **zanim dotkną Qt** (działa nawet bez wyświetlacza — SSH, CI). Wersja z jednego źródła (`ghostmean/__init__.py`), zero ręcznego wpisywania w wielu miejscach.
- **Prawdziwy pakiet `pytest`** (`tests/`, 97 testów, wszystkie zielone) zamiast dotychczasowych ręcznych skryptów jednorazowych z tej rozmowy:
  - `test_geometry.py` (36 testów) — `compute_panel_stations`/`compute_stations`, MAC/Area na znanym przykładzie referencyjnym i na Twoim łańcuchu 250→200→140→80, ciągłość, celowy skok, panele 1–5, skos ujemny/zero/duży/blisko 90°, skrajne dane bez crasha.
  - `test_csv.py` (15 testów) — round-trip, kanoniczne mm niezależnie od jednostki, zaokrąglenia do 6 miejsc, brakująca kolumna/pusty plik → kontrolowany wyjątek, plus `tests/fixtures/test.csv`, `test2.csv`, `test_err.csv` jako trwałe fixtury regresyjne.
  - `test_validation.py` (11 testów) — ostrzeżenia GUI napędzane bezpośrednio przez `test_err.csv`, reset projektu czyści ostrzeżenia.
  - `test_pdf_drawing.py` (19 testów) — PDF dla 1–5 paneli, realny skok, oba jednostki, ekstremalny skos, brak/zdegenerowane dane, `draw_wing_plan()` (wspólny renderer ekranu i PDF) osobno na `QPixmap`.
  - `test_i18n.py` (13 testów) — zgodność kluczy/placeholderów we wszystkich 6 językach, oraz **żywy test GUI** przełączający wszystkie języki na działającym `MainWindow` i sprawdzający brak surowych, nieprzetłumaczonych kluczy w interfejsie.
  - `test_version.py` (3 testy) — `pyproject.toml` i `__init__.py` muszą się zgadzać (dokładnie ten błąd, który już raz się zdarzył), `--version` faktycznie działa w prawdziwie headless subprocess (bez `QT_QPA_PLATFORM`).
- **Build z czystego checkoutu zweryfikowany naprawdę**: skopiowałem cały projekt do zupełnie nowego katalogu, potwierdziłem (tymczasowym unikalnym numerem wersji), że `python3 -c "import ghostmean"` faktycznie czyta lokalną kopię, nie globalnie zainstalowany pakiet — dopiero wtedy uruchomiłem `./scripts/build_appimage.sh` bez wcześniejszego `pip install -e .`, dokładnie jak w `git clone → cd → ./scripts/build_appimage.sh`. Zbudowany plik uruchomił się poprawnie.
- `pyproject.toml`: dodana grupa `[project.optional-dependencies] test = ["pytest>=7.0"]` i `[tool.pytest.ini_options]`.
...
## Aneks: Co dalej (odłożone do po v1.0)
